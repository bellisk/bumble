...
def tagslist(value):
    ...
    l = (
        '<div class="tagslist">'
        + "\n".join(
            '<a href="{url}" class="listtag"><span class="tagtitle">{title}</span> <span class="tagsize">{size}</span></a>'.format(
                url=reverse("entry", args=["tag/" + tag.name]),
                title=tag.nice_title(),
                size=tag.num_members(),
            )
            for tag in tags
        )
        + "</div>"
