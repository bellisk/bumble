...
def get_tag_entries(tags, path):
    return (
        tag_filter(tags, Entry.objects.filter(path__startswith=path + "/"))
        .filter(created__lte=now())
        .order_by("-created")
    )
...
def get_entry(path, request):
    ...
    if entry.created > now() and not (
        "preview" in request.GET and request.GET["preview"] == entry.magic_number
    ):
        raise Http404
    return entry
...
def entry(request, path):
    def entry_url(p):
        return request.build_absolute_uri(
            ensure_trailing_slash(reverse("entry", args=[urlify_path(p)]))
        )
    ...
    try:
        ...
        return redirect(
            link=entry_url(redirection.redirect_to), permanent=redirection.permanent
        )
    except:
        pass
    ...
    if path.endswith("/feed"):
        ...
        entries = Entry.objects.filter(
            path__startswith=path + "/", created__lte=now()
        ).order_by("-created")
        for e in entries:
            add_feed_entry(feed, e, extract_lead_and_content)
        return HttpResponse(
    ...
    if request.method == "POST":
        ...
        try:
            recaptcha_result = verify_recaptcha(
                request.META["REMOTE_ADDR"], request.POST["g-recaptcha-response"]
            )
        except:
        ...
        if not recaptcha_result[0]:
            ...
            if form.is_valid():
                ...
                try:
                    mail_admins(
                        "New comment by " + new_comment.commenter,
                        new_comment.text,
    ...
    c = {
        ...
        "descendents": Entry.objects.filter(
            path__startswith=path + "/", created__lte=now()
        ).order_by("-created")[0:PAGINATION],
        "pagination_url": reverse("page", args=[578329023, urlify_path(path)]),
        "feed_url": entry_url(path) + "feed",
        "user": request.user,
    }
...
def page(request, from_index, path):
    ...
    entries = entries[
        int(from_index) * PAGINATION : int(from_index) * PAGINATION + PAGINATION
    ]
    return HttpResponse(
        json.dumps(
            [
                {
                    ...
                    "created": formats.date_format(e.created, "DATETIME_FORMAT"),
                    "description": md(filepaths(e.lead)),
                    "link": entry_url(e.path),
                }
                for e in entries
...
def verify_recaptcha(ip, response):
    r = requests.post(
        ...
        {"secret": RECAPTCHA_PRIVATE, "remoteip": ip, "response": response},
    )
    if r.json()["success"]:
        return (True, None)
    else:
